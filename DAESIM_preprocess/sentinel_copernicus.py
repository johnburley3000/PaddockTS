...
# Process the TIFF file into an xarray DataArray
with rasterio.open("sentinel_output.tif") as src:
    data = src.read()
    coords = {
        "band": query["band_names"],
        "y": np.linspace(src.bounds.top, src.bounds.bottom, src.height),
        "x": np.linspace(src.bounds.left, src.bounds.right, src.width)
    }
    return xr.DataArray(data, coords=coords, dims=["band", "y", "x"])
