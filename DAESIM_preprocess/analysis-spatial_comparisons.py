...
pd.DataFrame(total_ndvi_flattened).describe()

np.percentile(total_ndvi_flattened, 67)
...
adjacent_mask = scipy.ndimage.binary_dilation(tree_mask | (total_ndvi_summed > 0.4), structure=structuring_element)
...
# productivity_score1 = ndvi.where(~adjacent_mask & (total_ndvi_summed < 0.4))
